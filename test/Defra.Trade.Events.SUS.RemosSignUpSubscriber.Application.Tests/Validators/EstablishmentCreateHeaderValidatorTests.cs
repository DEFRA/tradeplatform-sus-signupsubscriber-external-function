...
public sealed class EstablishmentCreateHeaderValidatorTests : ValidatorTestBase<MessageHeader>
{
    private readonly EstablishmentCreateMessageHeaderValidator _sut;

    public EstablishmentCreateHeaderValidatorTests()
    {
        _sut = new EstablishmentCreateMessageHeaderValidator();
    }
    ...
    public static TheoryData<ModelSetter[], ErrorDetails[]> GetTestData()
    {
        ...
        return new()
        {
            ...
            { new[] { Set(x => x.Label, null) }, new ErrorDetails[] { new("Label", "Label must be sus.remos.establishment.create") } },
            { new[] { Set(x => x.Label, "abc") }, new ErrorDetails[] { new("Label", "Label must be sus.remos.establishment.create") } },
            ...
            { new[] { Set(x => x.SchemaVersion, null) }, new ErrorDetails[] { new("SchemaVersion", "Schema Version must be 3") } },
            { new[] { Set(x => x.SchemaVersion, "abc") }, new ErrorDetails[] { new("SchemaVersion", "Schema Version must be 3") } },
            { new[] { Set(x => x.Status, null) }, new ErrorDetails[] { new("Status", "Status must be Created") } },
            { new[] { Set(x => x.Status, "abc") }, new ErrorDetails[] { new("Status", "Status must be Created") } },
            { new[] { Set(x => x.Status, "created") }, new ErrorDetails[] { new("Status", "Status must be Created") } },
    ...
    public void Validate_ReturnsTheExpectedErrors(ModelSetter[] setters, ErrorDetails[] expected)
    {
        ...
        var header = new MessageHeader
        {
            ...
            FullMessage = null,
            Label = "sus.remos.establishment.create",
