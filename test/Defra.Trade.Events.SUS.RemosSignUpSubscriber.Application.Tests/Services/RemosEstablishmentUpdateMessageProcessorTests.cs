...
public sealed class RemosEstablishmentUpdateMessageProcessorTests
{
    private readonly ICrmClient _client;
    private readonly ILogger<RemosEstablishmentUpdateMessageProcessor> _logger;
    private readonly IMapper _mapper;
    private readonly RemosEstablishmentUpdateMessageProcessor _sut;

    public RemosEstablishmentUpdateMessageProcessorTests()
    {
        _client = A.Fake<ICrmClient>(opt => opt.Strict());
        _mapper = A.Fake<IMapper>(opt => opt.Strict());
        _logger = A.Fake<ILogger<RemosEstablishmentUpdateMessageProcessor>>();
        _sut = new RemosEstablishmentUpdateMessageProcessor(_client, _mapper, _logger);
    }
    ...
    public async Task ProcessAsync_CreatesDynamicsAndReturnsAStatusResponseWhichDoesntForwardTheMessage()
    {
        ...
        A.CallTo(() => _client.UpdateAsync(inspectionLocation, default)).Returns(updateResultTask);
        ...
        A.CallTo(() => _client.UpdateAsync(inspectionLocation, default))
            .MustHaveHappenedOnceExactly();
    }
    ...
    public async Task ProcessAsync_WithCrmClientError_ThrowsException()
    {
        ...
        A.CallTo(() => _mapper.Map<InspectionLocation>(request)).Returns(inspectionLocation);
        A.CallTo(() => _client.UpdateAsync(inspectionLocation, default)).Throws(new ArithmeticException());
